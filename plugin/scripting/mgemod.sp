...
{	
	...
	if(isMapAm)
	{
		for (new i = 0;i<=g_iArenaCount;i++)
		{
			...
			if(g_bArenaKoth[i] || g_bArenaHasCap[i])
			{
				g_iCapturePoint[i] = -1;
			}
		}
					...
					if (KvGotoFirstSubKey(kv))
					{
						do
						{
							...
							if (KvGetNameSymbol(kv, "cap", id)) {
								KvGetString(kv, "cap", spawn, sizeof(spawn));
								count = ExplodeString(spawn, " ", spawnCo, 6, 16);
								g_bArenaHasCap[g_iArenaCount] = true;
								for (i=0; i<3; i++)	{
									g_fArenaCapPointOrigin[g_iArenaCount][i] = StringToFloat(spawnCo[i]);
								}
								LogMessage("Loaded cap point on arena %s.", g_sArenaOriginalName[g_iArenaCount]);
							} else {
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 0;i<=g_iArenaCount;i++)
	{
		...
		if(g_bArenaKoth[i] || g_bArenaAllowKoth[i])
		{
			...
			if(g_bArenaHasCap[i]) {
				point_loc[0] = g_fArenaCapPointOrigin[i][0];
				point_loc[1] = g_fArenaCapPointOrigin[i][1];
				point_loc[2] = g_fArenaCapPointOrigin[i][2];
			} else {
