...
new String:g_sArenaOriginalName[MAXARENAS+1][64],
    ...
    bool:g_bArenaAllowChange[MAXARENAS+1],
    bool:g_bArenaAllowKoth[MAXARENAS+1],
    bool:g_bArenaKothTeamSpawn[MAXARENAS+1],
    bool:g_bArenaHasCap[MAXARENAS+1],
    bool:g_bArenaHasCapTrigger[MAXARENAS+1],
    bool:g_bArenaShowHPToPlayers[MAXARENAS+1],
    bool:g_bArenaUltiduo[MAXARENAS+1],
    bool:g_bArenaKoth[MAXARENAS+1],
    bool:g_bArenaBoostVectors[MAXARENAS+1],
    ...
    g_iArenaCapTriggerEntity[MAXARENAS+1],
    g_iArenaCapEntity[MAXARENAS+1],
...
{
  name = "MGEMod",
  author = "Lange & Cprice; based on kAmmomod by Krolus, Modified by PepperKick, Extra modiftications by Nacho Replay and Obla",
  description = "Duel mod with realistic game situations. Originally modified from MGEMod by Lange & Cprice plugin for chilly maps.",
  version = PL_VERSION,
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:hItem) {
    if(IsValidClient(client)) {
        if(TF2_GetPlayerClass(client) == TFClass_Pyro) {
            if(iItemDefinitionIndex == 1179 || strcmp(classname, "tf_weapon_rocketpack", true) == 0) {
...
/* OnTriggerTouch(output, caller, activator, delay)
*
* When the cap trigger is touched
* ------------------------------------------------------------------------- */
...
public Action:Command_OneVsOne(client, args)
{
    if (!IsValidClient(client))
        return Plugin_Continue;
...
public Action:Command_TwoVsTwo(client, args)
{
    if (!IsValidClient(client))
        return Plugin_Continue;
...
public Action:Command_Koth(client, args) {
    if (!IsValidClient(client))
        return Plugin_Continue;
...
public Action:Command_Mge(client, args) {
    if (!IsValidClient(client))
        return Plugin_Continue;
...
public Action:Event_PlayerHurt(Handle:event,const String:name[],bool:dontBroadcast)
{
    ...
    if (attacker > 0 && victim != attacker) // If the attacker wasn't the person being hurt, or the world (fall damage).
    {
        ...
        if(g_bArenaEndif[arena_index] || g_bArenaBoostVectors[arena_index])
        {
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    for (new i = 0;i<=g_iArenaCount;i++)
    {
        ...
        if(g_bArenaKoth[i] || g_bArenaAllowKoth[i])
        {
            new Float:point_loc[3];
            new String:buffer[64];
            new entity;

            if(g_bArenaHasCapTrigger[i]) {
                entity = 0;
                while((entity = FindEntityByClassname(entity, "trigger_capture_area")) != -1)
                {
                    GetEntPropString(entity, Prop_Data, "m_iName", buffer, sizeof(buffer));

                    if(StrEqual(buffer, g_sArenaCapTrigger[i], false))
                    {
                        g_iArenaCapTriggerEntity[i] = entity;

                        PrintToServer("Found cap trigger entity %s for arena %s", g_sArenaCapTrigger[i], g_sArenaName[i]);
                ...
                if (g_bArenaHasCap[i]) {
                    ...
                    while((entity = FindEntityByClassname(entity, "prop_dynamic")) != -1)
                    {
                        ...
                        if(StrEqual(buffer, g_sArenaCap[i], false))
                        {
                            g_iArenaCapEntity[i] = entity;

                            PrintToServer("Found cap entity %s for arena %s", g_sArenaCap[i], g_sArenaName[i]);
                            break;
                        }
                    }
                }
...
public Action:Timer_Tele(Handle:timer, any:userid)
{
    ...
    else if(g_bArenaKoth[arena_index] && g_bArenaKothTeamSpawn[arena_index]) {
        new random_int;
        new offset_high, offset_low;
        if(g_iPlayerSlot[client] == SLOT_ONE || g_iPlayerSlot[client] == SLOT_THREE)
        {
...
public Action:Timer_StartDuel(Handle:timer, any:arena_index)
{
    ...
    if(g_bArenaKoth[arena_index])
    {
        ...
        if (g_bArenaHasCap[arena_index])
            SetEntProp(g_iArenaCapEntity[arena_index], Prop_Send, "m_nSkin", 0);
    }
    ...
    if(g_fKothCappedPercent[arena_index] >= 100)
    {
        ...
        if(g_bFourPersonArena[arena_index])
        {
            ...
            if(g_iPointState[arena_index] == TEAM_RED)
            {
                ...
                if (g_bArenaHasCap[arena_index])
                    SetEntProp(g_iArenaCapEntity[arena_index], Prop_Send, "m_nSkin", 1);
            }
            ...
            else if(g_iPointState[arena_index] == TEAM_BLU)
            {
                ...
                if (g_bArenaHasCap[arena_index])
                    SetEntProp(g_iArenaCapEntity[arena_index], Prop_Send, "m_nSkin", 2);
            }
